# Computational-Methods-for-Stochastic-DNA-Methylation-Dynamics
## Abstract
This repository contains the Approximate Chemical Master Equation (CME), Exact CME, and Stochastic Simulation Algorithm (SSA) codes used in the paper [A Comparison of Computational Methods for Modeling Stochastic Collaborative DNA Methylation Dynamics](https://www.biorxiv.org/content/10.1101/2025.11.25.690568v1) as well as a breif introduction on how to run the codes.

## File Description
#### CMEApproxModel.py
#### CMEExactModel.py
#### SSAModel.py
are codes found in the Models folder that contain the Approximate CME Model, Exact CME Model, and SSA Model codes, respectively, as functions, for modeling collaborative DNA methylation dynamics. Another python script can be used to call each function under various parameters as is done in the SSA Vs. CMEs Validation code.

